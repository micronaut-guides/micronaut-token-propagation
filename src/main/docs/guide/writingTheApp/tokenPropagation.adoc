As you can see propagating the JWT token to other microservices in our app complicates the code.
...
<1> Micronaut ships with several https://docs.micronaut.io/latest/api/io/micronaut/security/token/writer/TokenWriter.html[TokenWriter] beans which you can enable. In this example, we enable the HTTP header token writer which propagates the token in the HTTP Header `Authorization`. You can code your own `TokenWriter` to satisfy your requirements.
